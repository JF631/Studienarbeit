...
\subsubsection*{Pre-processing via filter application}
...
These vectors are, in case of the arm- and knee angles, given by the detected
body key point positions (see
...
The second vector represents the athletes' \ac{CM} velocity.
It is calculated based on the takeoff frame, which needs to be determined
first (see \autoref{subsec:4_takeoff_detection}).
...
rate\footnote{Chosen as $1/10$ of the frame rate}.
...
\begin{figure}[!h]
    ...
    \begin{subfigure}{0.4\textwidth}
        \includegraphics[scale=0.09]{takeoff_angle_overlay.jpg}
        \caption{Poses shown as overlay}
    \end{subfigure}
    \begin{subfigure}{0.4\textwidth}
        \includegraphics[scale=0.09]{takeoff_angle_no_overlay.jpg}
