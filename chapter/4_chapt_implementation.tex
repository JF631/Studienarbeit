...
\subsubsection{Connection between the ground station and the on-board computer}\label{subsubsec:4_pi_wifi}
%TODO: insert connection diagram / figure (?)%
...
%TODO: explain the actual implementation on  the Raspberry Pi%

\subsubsection{TCP sockets for communication}\label{subsubsec:4_udp_sockets}
Even though the video recording is performed by the on-board RaspberryPi
computer, the recording must be initializable by the user on the ground
station laptop.
...
RaspberryPi to the ground station, the connection- and confirmed orientated
\ac{TCP} connection is chosen over the connectionless
\ac{UDP}~\cite{SpecificationInternetTransmission1974} communication to avoid
data loss.\\\\
\noindent When the ground station connects to the drone, a \ac{TCP} socket is
opened in a background thread.
The corresponding \ac{TCP} connection is initialized using a three-way
handshake\footnote{The request is sent by the ground station, the drone
acknowledges, and the ground station acknowledges the drone's response} by the
ground station.
...
\noindent When a video recording has ended (either because the user manually
