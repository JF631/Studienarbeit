...
It is divided into the development of the analysis software and the
development and assembly of the drone.\\
First, the analysis software implementation is explained including the
video processing, the development of an automatic takeoff frame detection and
and the handling of analysis results.\\
Afterwards, the drone's hardware assembly, flight related as well as camera
related software parts are implemented.\\
The drone related controls and the jump analysis process are combined into one
\ac{GUI}.
...
\begin{table}[h!]
    ...
    \hline
    Dimensions\newline(W x H x L) in mm & $50 \times 15.5 \times 81.5$ & $55 \times 65 (W \times L)$\\ 
    \hline
    Other & Failsafe options (e.g. extra power supply, \acs{GPS}, etc.) & None\\
    \hline
    Price (Eur.) & about 120 & 200 \\
    \hline
    \end{tabular}
...
\autoref{subsec:4_pi_wifi} (and following).\\\\
...
\autoref{subsec:4_live_stream}) are combined into one \ac{GUI} control
