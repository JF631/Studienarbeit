...
frame detection is even more crucial.
...
each analysis \autoref{fig:4_angles_height_plot}).\\
The presented data has not been cleaned up in any way.
As can be seen especially in the first and last analysis, the position and
angle data is not accurate due to body key point detection inaccuracies.
However, afterwards the key points are detected correctly during the approach.\\\\
...
First, a mathematical description of the \ac{CM} position during the jump is
implemented.
...
The T matrix in \autoref{eq:linear_reg_matrix} contains all the frame
...
\end{equation}
where $r = PT_1 - 1 - PT_0$.
The now following steps are equal to the linear case.
To measure the error, the \ac{SSE} function is used again.
As the matrix notation does not change, the following calculations are set-up
equally to equations~\ref{eq:sse_matrix} to~\ref{eq:normal_equation}.
The solution can again be analytically found by the normal equation
\autoref{eq:normal_equation}:
\[
    \vec{\beta} = (T^T T)^{-1}(T^T\vec{h}) 
\] 
