...
This algorithm is based on the algorithm for a takeoff frame detection
...
\noindent The shown algorithm was then applied to all three datasets shown in
\autoref{fig:4_angles_height_plot}. 
...
datasets which is the only parameter which is relevant for the takeoff
detection.
The data is still not cleaned up in any way, thus, outliers are still
visible.
However, as can be seen, the automatically detected takeoff frames are very
