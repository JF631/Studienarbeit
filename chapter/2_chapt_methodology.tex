...
All common platforms including the Linux, Windows and MacOS are supported by 
Qt.
...
Like explained earlier, Qt is based on the programming language C++.
As justified in \autoref{subsec:2_programming_language} however, Python is 
chosen as programming language for this project.
To combine the Python's machine learning advantages with Qt's 
platform-independent \ac{GUI} development options, the module \textit{PyQt5}
\footnote{\url{https://pypi.org/project/PyQt5/}} is used.\\
This module allows building Qt applications using Python instead of C++. 
Every other described advantage Qt offers remains valid with the use of PyQt.
